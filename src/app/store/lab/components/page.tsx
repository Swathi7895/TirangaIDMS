...
const API_BASE_URL = 'http://localhost:8080/api/store/lab/components';
...
export default function LabComponentsPage() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setNewComponent({ name: '', category: '', quantity: 0, location: '', itemCondition: 'New' });
    } catch (error) {
      console.error('Error adding component:', error);
      alert(`Error adding component: ${(error as Error).message}`);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      alert(`Error updating component: ${(error as Error).message}`);
    }
  };
